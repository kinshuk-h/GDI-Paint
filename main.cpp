...
HINSTANCE hInst;
static COLORREF CFC = RGB(0,0,0), CBC = RGB(255,255,255);
static COLORREF CBCO,CFCO;
...
INT_PTR CALLBACK DlgProcB(HWND hWndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch(uMsg)
    {
            ...
            case ID_OK:
                {
                    ...
                    GetWindowText(hR,R,3);
                    GetWindowText(hG,G,3);
                    GetWindowText(hB,B,3);
                    int r = atoi(R),g = atoi(G),b = atoi(B);
                    ::CBC = RGB(r,g,b);
                    EndDialog(hWndDlg,0);
                    break;
        ...
        {
            HBRUSH hbr;
            if (Apply && ((HWND)lParam == hC))
            {
                SetBkColor((HDC) wParam, ::CBC);
                SetTextColor((HDC) wParam, ::CBC);
            }
            Apply = false;
            HBRUSH br = CreateSolidBrush(::CBC);
            ::CBC = CBCO;
